...
const loginLimiter = rateLimit({
    ...
    message: { 
        message: 'تم تجاوز الحد المسموح لمحاولات تسجيل الدخول. الرجاء المحاولة بعد 15 دقيقة.' 
    },
...
router.post('/login', loginLimiter, async (req, res) => {
    try {
        ...
        if (admin.email === email && isMatch) {
            ...
            res.status(401).json({ message: 'البريد الإلكتروني veya كلمة المرور غير صحيحة' });
        }
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
...
router.get('/fix-admin', async (req, res) => {
    try {
        ...
        res.json({ message: '✅ تم تصفير حساب الآدمن بنجاح! روح جرب هلا بـ 12345' });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
});
...
router.post('/contact', auth, async (req, res) => {
    try {
        ...
        if (contact) {
            ...
            contact = new Contact({ email, phone, address, mapUrl, instagram, linkedin, github });
            await contact.save();
        }
